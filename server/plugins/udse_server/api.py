...
class API:
    def ping(self):
        ...
        ПРОВЕРКА ДОСТУПНОСТИ СЕРВЕРА
    	    ВОЗВРАЩАЕТ:
    	    True [bool]
            False [bool] - в случае недоступности базы данных
        '''
        try:
            dbConnect()
            result = True
    ...
    def setupDB(self, auth):
        ...
        try:
            ...
            query += 'CREATE TABLE "{}"(id SERIAL PRIMARY KEY, data JSONB, security JSONB); '.format('log')
    ...
    def setUser(self, auth, userID, userData):
        ...
        try:
            ...
            privacy = 0
            if 'privacy' in userData.keys() and userData['privacy']:
    ...
    def delUser(self, auth, userIDs):
        ...
        try:
            ...
            for userID in userIDs:
                if userID == 1:
                    raise Exception('Нельзя удалять системного пользователя!')
                elif userID == getAccess(auth, 'user', dataID=userID, write=True):
    ...
    def setSecurity(self, auth, secData, dataType='security', dataID=None):
        ...
        try:
            ...
            print str(error).decode('utf8')
            raise
        secDataBackup = dict(secData)
        if privacy != secData['privacy']:
        ...
        try:
            ...
            try:
                ...
                if dataID:
                    query = "UPDATE \"{}\" SET security='{}' WHERE id='{}'".format(dataType, json.dumps(secDataBackup), dataID)
                else:
                    query = "UPDATE \"group\" SET security='{}' WHERE id='{}'".format(json.dumps(secDataBackup), dataType)
                cursor.execute(query)
                connection.commit()
                raise Exception('Изменения в безопасности приведут к утрате доступа!')
        except Exception as error:
            if query:
                print query.decode('utf8')
            print str(error).decode('utf8')
...
####            getAccess(auth, 'group', dataType)
    ...
    def delData(self, auth, dataType, dataIDs):
        ...
        try:
            ...
            if len(fetch) > 0:
                ...
                for dataID in fetch:
                    query += " (info->>'dataType'='{}' AND info->>'dataID'='{}') OR".format(dataType, dataID[0])
                query = query.rstrip(' OR')
                query += " RETURNING id"
                cursor.execute(query)
            else:
    ...
    def addFile(self, auth, dataType, dataID, fileData, fileInfo={}):
        ...
        # todo Дублирование файлов

        try:
            # Проверка наличия родительских объектов для добавляемых файлов
            getAccess(auth, dataType, dataID, write=True)
        except Exception:
            raise Exception('Не существует связанного объекта типа "{}" с ID {}!'.format(dataType, dataID))
        try:
        ...
        try:
            ...
            query = "UPDATE file SET info = '{}' WHERE id = '{}'".format(fileInfo, fileID)
            cursor.execute(query)
            connection.commit()
            result = fileID
        except Exception as error:
